...
class DemoSpace(Graph):
    ...
    def __init__(self):
        ...
        # input node
        input_node = self.append_node(name="input_node")

        # Stem
        stem = self.append_node(name="stem")
    ...
    def add_par_cell_with_skip(self, cell: Graph, input_idx: int, scope: str, repetitions: int, par_degree: int) -> int:
        ...
        for r in range(0, repetitions):
            ...
            # Merge node
            merge_node = self.append_node(name=f"add {scope} r{r}", comb_op=sum)
    ...
    #         tensor_inout = module(tensor_inout)
    #
    #     return tensor_inout
