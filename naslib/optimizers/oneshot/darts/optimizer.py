...
class DARTSOptimizer(MetaOptimizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.op_scaler = torch.cuda.amp.GradScaler()
    ...
    def step(self, data_train, data_val):
        ...
        if unrolled:
            ...
            self.arch_scaler.scale(val_loss).backward()
            self.arch_scaler.step(self.arch_optimizer)
            ...
            self.op_scaler.scale(train_loss).backward()
            # train_loss.backward()
            if self.grad_clip:
                torch.nn.utils.clip_grad_norm_(self.graph.parameters(), self.grad_clip)
            self.op_scaler.step(self.op_optimizer)
            self.op_scaler.update()
