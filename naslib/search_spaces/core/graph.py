...
class Graph(torch.nn.Module, nx.DiGraph):
    ...
    **Interface to tabular benchmarks**
    ...
    def append_node(self, **attr) -> int:
        """
        Add a new node to the graph, increments the current node_idx
        :param attr: node attributes
        :return: index of the new node
        """
        self.add_node(self.node_idx, **attr)
        idx = self.node_idx
        self.node_idx += 1
        return idx

    def add_edge_final(self, u: int, v: int, **attr):
        """
        Add an edge and finalize it
        :param u: from
        :param v: to
        :param attr: edge attributes
        """
        self.add_edge(u, v, **attr)
        self.edges[u, v].finalize()
    ...
    def forward(self, x, *args):
        ...
        for _, module in self.named_children():
            tensor_inout = module(tensor_inout, *args)
    ...
    def parse(self):
        ...
        for node_idx in lexicographical_topological_sort(self):
            node_name = node_idx
            if 'name' in self.nodes[node_idx]:
                node_name = self.nodes[node_idx]['name']

            if "subgraph" in self.nodes[node_idx]:
                self.nodes[node_idx]["subgraph"].parse()
                self.add_module(
                    "{}-subgraph_at({})".format(self.name, node_name),
                    self.nodes[node_idx]["subgraph"],
                )
            elif "comb_op" in self.nodes[node_idx]:
                if isinstance(self.nodes[node_idx]["comb_op"], torch.nn.Module):
                    self.add_module(
                        "{}-comb_op_at({})".format(self.name, node_name),
                        self.nodes[node_idx]["comb_op"],
                    )
            for neigbor_idx in self.neighbors(node_idx):
                neighbor_name = neigbor_idx
                if 'name' in self.nodes[neigbor_idx]:
                    neighbor_name = self.nodes[neigbor_idx]['name']
                ...
                if isinstance(edge_data.op, Graph):
                    ...
                    for primitive in edge_data.op.get_embedded_ops():
                        if isinstance(primitive, Graph):
                            primitive.parse()
                self.add_module(
                    "{}-edge({},{})".format(self.name, node_name, neighbor_name),
                    edge_data.op,
                )
        self.is_parsed = True
