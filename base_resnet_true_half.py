...
class BaseResNet(nn.Module):
    def __init__(
        ...
        groups = 1
        blocks = []
        blocks.append(SmallStem(in_channels, 160))
        for i in range(s0_depth):
            blocks.append(Inception_ResNet_A_Half(160, 0.17, groups))
        blocks.append(Small_Reduction(160, 544))
        for i in range(s1_depth):
            blocks.append(Inception_ResNet_B_Half(544, 0.10, groups))
        blocks.append(Small_Reduction(544, 1040))
        for i in range(s2_depth - 1):
            blocks.append(Inception_ResNet_C_Half(1040, 0.20, groups))
        if s2_depth > 0:
            blocks.append(Inception_ResNet_C_Half(1040, scale=0.20, activation=False))
        self.features = nn.Sequential(*blocks)
        self.conv = Conv2d(
            1040, 768, 1, stride=1, padding=0,
            bias=False
        )
